...
function showUsage() {
    echo ""
    echo "Usage:"
    echo "  oam-med.sh [--context <context>] --operation <operation> [--mode <mode>] --pods <pod_list>|--group <pod_group>"
    echo ""
    echo "Note:"
    ...
    echo ""
    echo "Supported values for argument <context>:"
    echo "  fixe-dev   : Fixe Dev environment"
    echo "  fixe-int   : Fixe Int environment"
    echo "  fixe-prod  : Fixe Prod environment"
    echo
    ...
    echo "  With operation: 'cleanup', 'softStop':"
    echo "    imsr7  : all IMS R7 pods"
    echo "    imsr10 : all IMS R10 pods"
    echo "    imsl1  : all IMS L1 pods"
    echo "    imsl2  : all IMS L2 pods"
    echo "    imsall : all IMS pods"
    echo "    vtcl1  : all VTC L1 pods"
    echo "    vtcl2  : all VTC L2 pods"
    echo "    vtcall : all VTC pods"
    echo "  With operation: 'helmDelete':"
    echo "    imsl1  : IMS L1 chart"
    echo "    imsl2  : IMS L2 chart"
    echo "    imsall : IMS L1 and L2 charts"
    echo "    vtcl1  : VTC L1 chart"
    echo "    vtcl2  : VTC L2 chart"
    echo "    vtcall : VTC L1 and L2 charts"
    echo "  With operation: 'refreshCache':"
    echo "    imsl2  : IMS L2 pods"
    echo "    vtcl2  : VTC L2 pods"
    echo ""
    echo "----"
}
    ...
    "refreshCache")
        case "$POD_GROUP" in
            ...
            "none" )
                ;;
            *)
                logError "For operation 'refreshCache', only groups 'imsl2' and 'vtcl2' are supported."
                showUsage
                exit 1
                ;;
        esac
    ...
    "fixe")
        case "$POD_GROUP" in
            ...
            "imsr10")
                ...
                ;;
            "imsl1")
                POD_LIST=$(kubectlWrapperSilent --kubeconfig=$HOME/K8S/kube.config --context $K8S_CONTEXT get pods | grep "^ims-" | awk '{ print $1 }' | tr '\n' ',' | sed 's/,$//' )
            ...
            "imsl2")
                ...
                STATEFUL_SET_LIST="mu-ims-proc"
                ;;
            "imsall")
                POD_LIST=$(kubectlWrapperSilent --kubeconfig=$HOME/K8S/kube.config --context $K8S_CONTEXT get pods | grep -e "^ims-" -e "^mu-ims-" | awk '{ print $1 }' | tr '\n' ',' | sed 's/,$//')
                STATEFUL_SET_LIST="ims-r7,ims-r10,mu-ims-proc"
                ;;
            "vtcl1")
                POD_LIST=$(kubectlWrapperSilent --kubeconfig=$HOME/K8S/kube.config --context $K8S_CONTEXT get pods | grep "^vtc" | awk '{ print $1 }' | tr '\n' ',' | sed 's/,$//' )
                STATEFUL_SET_LIST="vtc-pfvn"
                ;;
            "vtcl2")
                POD_LIST=$(kubectlWrapperSilent --kubeconfig=$HOME/K8S/kube.config --context $K8S_CONTEXT get pods | grep "^mu-vtc-" | awk '{ print $1 }' | tr '\n' ',' | sed 's/,$//' )
                STATEFUL_SET_LIST="mu-vtc-proc"
                ;;
            "vtcall")
                POD_LIST=$(kubectlWrapperSilent --kubeconfig=$HOME/K8S/kube.config --context $K8S_CONTEXT get pods | grep -e "^vtc-" -e "^mu-vtc-" | awk '{ print $1 }' | tr '\n' ',' | sed 's/,$//')
                STATEFUL_SET_LIST="vtc-pfvn,mu-vtc-proc"
                ;;
            "none")
                if [[ "$POD_LIST" = "none" ]]; then
